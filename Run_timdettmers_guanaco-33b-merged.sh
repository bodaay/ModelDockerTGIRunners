...
docker run -d  --name $container_name --gpus all --shm-size $shm_size -p $port:80 -v /home/ubuntu/Storage/:/data -e QUANTIZE=gptq -e GPTQ_BITS=4 -e GPTQ_GROUPSIZE=128 ghcr.io/huggingface/text-generation-inference:latest --model-id /data/timdettmers_guanaco-65b-merged/ --num-shard $num_shard
